...
Follow.getFollowersById = function (id) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      You'll need: id
      ===============================================*/
      let [followers] = await db.execute("SELECT username, avatar FROM follows JOIN users ON follows.authorId = users._id WHERE followedId = ?", [
        id,
      ]);
      resolve(followers);
    } catch {
...
Follow.getFollowingById = function (id) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      You'll need: id
      ===============================================*/
      let [followers] = await db.execute("SELECT username, avatar FROM follows JOIN users ON follows.followedId = users._id WHERE authorId = ?", [
        id,
      ]);
      resolve(followers);
    } catch {
