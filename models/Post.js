...
Post.findSingleById = function (id, visitorId = 0) {
  return new Promise(async function (resolve, reject) {
    ...
    let [[post]] = await db.execute(
      "SELECT title, body, posts._id, posts.author, createdDate, username, avatar FROM posts JOIN users ON posts.author = users._id WHERE posts._id = ?",
      [id]
    );
...
Post.findByAuthorId = async function (authorId) {
  ...
  let [posts] = await db.execute(
    "SELECT title, body, posts._id, posts.author, createdDate, username, avatar FROM posts JOIN users ON posts.author = users._id WHERE posts.author = ? ORDER BY createdDate DESC",
    [authorId]
  );
  return posts;
};
...
Post.countPostsByAuthor = function (id) {
  return new Promise(async (resolve, reject) => {
    ...
    You'll need: id
    ===============================================*/
    const [[{ posts }]] = await db.execute("SELECT COUNT(posts._id) as posts FROM posts WHERE posts.author = ?", [id]);
    resolve(posts);
  });
};
...
Post.search = function (searchTerm) {
  return new Promise(async (resolve, reject) => {
    if (typeof searchTerm == "string") {
      ...
      ===============================================*/
      let [posts] = await db.execute(
        "SELECT title, body, posts._id, posts.author, createdDate, username, avatar FROM posts JOIN users ON posts.author = users._id WHERE MATCH(title, body) AGAINST(?)",
        [searchTerm]
      );
      resolve(posts);
    } else {
...
Post.getFeed = async function (id) {
  ...
  ===============================================*/
  let [posts] = await db.execute(
    "SELECT posts._id, title, body, createdDate, username, avatar FROM posts JOIN users ON posts.author = users._id WHERE author IN (SELECT followedId FROM follows WHERE authorId = ?) ORDER BY createdDate DESC",
    [id]
  );
